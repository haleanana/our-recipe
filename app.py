...
def home():
    return render_template(
        "index.html", recipes=mongo.db.recipes.find().sort(
            'added_on', -1).limit(3),
        views=mongo.db.recipes.find().sort('views', -1).limit(3))
...
def recipes():
    ...
    return render_template("recipes.html", now=now.strftime(
        '%d %B %Y'), recipes=recipes)
...
def show_recipe(recipe_id):
    ...
    return render_template(
        'showrecipe.html', recipe=my_recipe, categories=all_categories)
...
def add_recipe():
    return render_template(
        "addrecipe.html", categories=mongo.db.categories.find())
...
def edit_recipe(recipe_id):
    ...
    all_categories = mongo.db.categories.find()
    return render_template(
        'editrecipe.html', recipe=my_recipe, categories=all_categories)
